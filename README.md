...
| --desc-by-guest              | Arrange descriptions by guest, showing just guest, implies --no-desc-by-(judge/staff/presenter) |
| --desc-by-judge              | Arrange descriptions by guest, showing just guest, implies --no-desc-by-(guest/staff/presenter) |
| --desc-by-presenter          | Arrange descriptions by presenters, implies --desc-by-(guest/judge/staff)                       |
| --desc-by-staff              | Arrange descriptions by staff, showing just staff, implies --no-desc-by-(guest/judge/presenter) |
...
| --file-by-guest              | Generate a file for each guest, implies --no-file-by-(judge/staff/presenter)                    |
| --file-by-judge              | Generate a file for each judge, implies --no-file-by-(guest/staff/presenter)                    |
| --file-by-presenter          | Generate a file for each presenter, implies --file-by-(guest/judge/staff)                       |
| --file-by-room               | Generate a file for each room                                                                   |
| --file-by-staff              | Generate a file for each staff, implies --no-file-by-(guest/judge/presenter)                    |
...
| --no-desc-by-judge           | Do not arrange by guest, exclude judge if --desc-by-presenter is given                          |
| --no-desc-by-presenter       | Do not arrange descriptions by presenter                                                        |
| --no-desc-by-staff           | Do not arrange by staff, exclude staff if --desc-by-presenter is given                          |
...
| --no-section-by-staff        | Do not generate a section for each staff                                                        |
...
| --section-by-judge           | Generate a section for each judge                                                               |
...
| --section-by-staff           | Generate a section for each staff                                                               |
...
| Prereq        |                        | A list of prerequisite panels, which will be listed after the description, should be the unique id and 3 digit panel id without any suffix     | FW001,FW002                       |
...
| J:Name=Group  |                        | Judge with name of Name, member of group. Set to non-blank ( such as "Yes" ) if required to be present, "*" if present but unlisted            | Yes                               |
| J:Name        |                        | Judge with name of Name. Set to non-blank ( such as "Yes" ) if required to be present, "*" if present but unlisted                             | *                                 |
| J:Other       |                        | Additional judges that don't have a header, you can just add rows if you want more columns                                                     | One Shot Wonder                   |
