...
class Presenter {    ## no critic (Modules::RequireEndWithOne,Modules::RequireExplicitPackage)
...
## use critic
    ...
    sub _decode_indices ( @values ) {
        return
            map { ref $_ ? ( _decode_indices( @{ $_ } ) ) : ( $_ ) } @values;
    }
    ...
    method compare ( $other, $swap ) {
        die qq{Compare Presenter with something else\n}
            unless $other isa Presenter;

        my $lhs = $swap ? $other : $self;
        my $rhs = $swap ? $self  : $other;

        my $res = $lhs->get_presenter_rank() <=> $rhs->get_presenter_rank();
        return $res if $res;

        # Compare indices from major to minor
        my @left_ind  = $lhs->get_index_array();
        my @right_ind = $rhs->get_index_array();
        for my $ind ( 0 .. max $#left_ind, $#right_ind ) {
            $res = ( $left_ind[ $ind ] // 0 ) <=> ( $right_ind[ $ind ] // 0 );
            return $res if $res;
        }

        return $lhs->get_presenter_name() cmp $rhs->get_presenter_name()
            || $lhs->get_pid() <=> $rhs->get_pid();
    } ## end sub compare

    sub lookup ( $class, $name_with_group, $index = undef, $rank = undef ) {
        defined $name_with_group
            or return;
        return if $name_with_group eq q{};

        if ( $name_with_group =~ s{\A (?<rank> \w ) : }{}xms ) {    ## no critic(RegularExpressions::ProhibitUnusedCapture)
