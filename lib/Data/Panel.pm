...
class Data::Panel :isa(TimeRange) {    ## no critic (Modules::RequireEndWithOne,Modules::RequireExplicitPackage,CodeLayout::ProhibitParensWithBuiltins)
    ...
    Readonly our $PREFIX_SHORT_ALPHA_MAX    => 2;
    Readonly our $PREFIX_SHORT_ALPHA_DIGITS => 3;
    Readonly our $PREFIX_LONG_ALPHA_DIGITS  => 2;
...
## use critic
    ...
    sub _uniq_to_base_type_remain ( $id ) {
        if ( $id =~ s{ \A (?<type> [[:alpha:]]++ ) (?<num> \d++ ) }{}xms ) { ## no critic(RegularExpressions::ProhibitUnusedCapture)
            my $type   = $+{ type };
            my $number = 0 + $+{ num };
            my $need_len
                = $PREFIX_SHORT_ALPHA_MAX >= length $type
                ? $PREFIX_SHORT_ALPHA_DIGITS
                : $PREFIX_LONG_ALPHA_DIGITS;
            $number =~ s{ \A 0* }{}xms;
            if ( $need_len > length $number ) {
                $number = ( q{0} x $need_len ) . $number;
    ...
    ADJUST {
        $presenter_set isa PresenterSet
